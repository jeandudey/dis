...
use goblin;
use falcon::architecture::*;
use falcon::error::*;
use falcon::loader::*;
use falcon::memory::backing::Memory;
...
impl Elf {
    ...
    pub fn new(bytes: Vec<u8>, base_address: u64) -> Result<Elf> {
        let architecture = {
            ...
            let architecture = if elf.header.e_machine == goblin::elf::header::EM_MIPS {
                match elf.header.endianness()? {
                    goblin::container::Endian::Big => {
                        Box::new(XtensaLx6::new(Mcu::esp32())) as Box<dyn Architecture>
                    }
    ...
    pub fn dt_needed(&self) -> Result<Vec<String>> {
        ...
        if let Some(dynamic) = elf.dynamic {
            ...
            for r#dyn in &dynamic.dyns {
                if r#dyn.d_tag == goblin::elf::dynamic::DT_STRTAB {
                    strtab_address = Some(r#dyn.d_val);
                    break;
                }
            }
            ...
            for section_header in &elf.section_headers {
                    ...
                    && section_header.sh_addr + section_header.sh_size > strtab_address
                {
                    ...
                    for r#dyn in dynamic.dyns {
                        if r#dyn.d_tag == goblin::elf::dynamic::DT_NEEDED {
                            let so_name = &strtab[r#dyn.d_val as usize];
